...
import type { Student, ClassData, EvaluationPeriod, Assignment, EvaluationCriterion, SpecificCompetence, KeyCompetence, AcademicConfiguration } from '../types';
            ...
            {isExpanded && (
                <div className="bg-slate-50 border-t border-slate-100 p-4">
                    {assignments.length === 0 ? (
                        <p className="text-slate-500 italic text-sm">No hay tareas en este periodo.</p>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {assignments.map(assignment => {
                                ...
                                return (
                                    <div key={assignment.id} className="bg-white p-3 rounded border border-slate-200 flex justify-between items-start">
                                        <div className="flex-1 min-w-0 pr-2">
                                            ...
                                            <p className="text-xs text-slate-500 mt-0.5">
                                                {category?.name} • {assignment.date ? new Date(assignment.date).toLocaleDateString() : 'Sin fecha'}
                                            </p>
                                        </div>
